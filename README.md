...
> This is a customized fork of the Isaac Lab framework (version 1.3.0). The main changes include:
> - Added contact locations and normal forces in the `ContactSensorData` class.
> - Calculate weighted contact point in a gpu-accelerated manner.
...
## Things to know
Due to network issues, you might be unable to access NVIDIA's Omniverse Nucleus. Here are the possible solutions:
- Follow the offcial [instructions](https://docs.omniverse.nvidia.com/isaacsim/latest/installation/install_faq.html#assets-and-nucleus) and download the assets manually to your local machine (be cautious, the file size is LARGE).
- Change line 684 in `source/extensions/omni.isaac.lab/omni/isaac/lab/app/app_launcher.py`:
   ```python
   assets_path = "<YOUR_LOCAL_ASSET_PATH>/NVIDIA/Assets/Isaac/4.2"
   ```
